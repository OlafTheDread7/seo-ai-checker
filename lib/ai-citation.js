...
/**
 * Ask Perplexity's Agent API a question and return the web-grounded answer plus
 * its sources. The Agent API (`/v1/agent`) returns web-grounded answers with
 * built-in citations in one call — exactly the "what an AI answer engine tells
 * a buyer, and what it cited" signal we want.
 *
 * Response shape (OpenAI-responses style):
 *   output: [
 *     { type: 'search_results', results: [{ title, url, snippet }] },  // grounding
 *     { type: 'message', content: [{ type: 'output_text', text, annotations }] }
 *   ]
 *
 * Returns { answer, sources } or { error }.
 */
export async function queryPerplexity(prompt, { apiKey, model = 'perplexity/sonar', timeoutMs = 40000 } = {}) {
  ...
  try {
    const res = await fetch('https://api.perplexity.ai/v1/agent', {
      ...
      body: JSON.stringify({
        ...
        instructions:
          'Answer as a helpful local-search assistant. Recommend real, specific ' +
          'businesses by name and include their official website links.',
      }),
    });
    if (!res.ok) {
      const t = await res.text().catch(() => '');
      return { error: `Perplexity Agent API returned HTTP ${res.status}${t ? ': ' + t.slice(0, 160) : ''}` };
    }
    const data = await res.json();
    if (data?.error) {
      const e = data.error;
      return { error: `Perplexity error: ${typeof e === 'object' ? e.message || JSON.stringify(e) : e}` };
    }

    const output = Array.isArray(data?.output) ? data.output : [];

    // Answer text = the assistant message content.
    const answer = output
    ...
    // Sources = the web results the engine grounded on, plus any URLs the answer
    // itself links to (annotations are often empty, so we do both).
    const sources = [];
    for (const o of output) {
      if (o && o.type === 'search_results' && Array.isArray(o.results)) {
        for (const r of o.results) if (r && r.url) sources.push(r.url);
      }
    }
    for (const m of answer.matchAll(/https?:\/\/[^\s)\]]+/g)) sources.push(m[0]);
 ...
 */
export async function runAiCitation({ finalUrl, html, apiKey, model = 'perplexity/sonar', onProgress }) {
  const identity = extractIdentity(finalUrl, html);
  const prompts = buildPrompts(identity);
  // Run the prompts concurrently — each Agent call does a live web search and
  // takes several seconds, so firing them in parallel keeps the whole check
  // well under any platform request timeout.
  let done = 0;
  const tick = (p, r) => {
    done++;
    if (onProgress) onProgress({ done, total: prompts.length, prompt: p.text });
    return r;
  };
  const results = await Promise.all(
    prompts.map(async (p) => {
      const q = await queryPerplexity(p.text, { apiKey, model });
      if (q.error) return tick(p, { ...p, error: q.error });
