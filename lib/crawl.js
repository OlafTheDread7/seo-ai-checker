...
export async function crawlAndScan({
  ...
  const pageReports = [];
  let done = 0;
  // Remember how the entered URL itself responded, so that if nothing is
  // scannable we can tell the user *why* (blocked, 404, non-HTML) instead of a
  ...
  while (queue.length && done < maxPages) {
    ...
    await Promise.all(
      batch.map(async (pageUrl) => {
        ...
        if (pageUrl === start.href && !startFetch) startFetch = page;
        done++;
        if (
          page.ok &&
          page.status < 400 &&
  ...
  return { pageReports, origin, startUrl: start.href, startFetch };
}
